...
description: >
  ...
  "전환률 알려줘", "퍼널 보여줘", "이 플로우 어디서 떨어지나", "화면 사용도 분석",
  "안 쓰이는 버튼 찾아줘", "audit", "감사", "click 낮은 element", "funnel-check",
  "이 화면 왜 안 쓰여" 같은 표현이 나오면 반드시 이 스킬을 사용할 것.
  두 가지 mode를 가짐 — flow(플로우 전환률)와 audit(화면 단면 element 사용도).
  사용자에게 한 단계씩 물어가며 정답을 함께 만들고 athena 쿼리는 항상 전문 보여주고
  승인 후 실행. PD가 PO와 차별화된 데이터 근거로 화면에 "넣기만" 하지 않고
  "뺄 수 있게" 돕는 것이 미션. 의존: log-explore / log-query / log-spec 스킬과
  athena mcp / log center mcp.

# 이 파일은 [YH] Data-insight 폴더의 WIP 사본입니다.
# 안정화 후 ../../skills/funnel-check/ 로 promotion 됩니다.
---

# 데이터 인사이트 측정 (funnel-check)

프로덕트 디자이너가 자기 오너십 화면의 데이터를 **정확하게** 측정하는 skill입니다. PD가 PO와 차별화된 데이터 근거를 들고, 화면에 "넣기만" 하는 게 아니라 사용도 낮은 element를 **뺄 수 있게** 돕는 게 이 skill의 미션입니다.
...
| "사용도", "클릭률", "안 쓰이는", "제거", "audit", "감사", "낭비", "이 화면의" | **audit** |
| 명시적 플래그 `--mode=flow` 또는 `--mode=audit` | 그 mode |
| 둘 다 / 모호 | 사용자에게 확인 |

추론 결과를 사용자에게 알리고 시작한다:
```
"입력을 보니 audit mode로 진행하면 될 것 같아. (이 화면의 element 사용도 분석)
...
audit mode는 **"이 화면에 있는 모든 element 중 사용도가 낮은 것을 찾아 제거 후보를 발굴"** 한다. 핵심은 두 source 대조:

- **명세 ∩ 로그**: 디자인도 됐고 사용자도 클릭하는 정상 element
- **명세 only**: 디자인은 됐는데 한 번도 안 클릭됨 → **강력한 제거 후보**
...
- 분석 목적은? (예: "리뉴얼 시 제거할 모듈 발굴", "안 쓰이는 버튼 찾기")
- 측정 기간은? (기본: 최근 7일)
...
⚠️ 명세 only (안 클릭됨, M개) ← 강력한 제거 후보
- old_promo_banner (MODULE)
- ...

⚡ 로그 only (비공식 element, K개) ← 명세 정비 필요
...
- **click_per_pv** = 클릭 UV / 페이지 PV UV — **제거 후보 판단의 핵심 지표**. 페이지 진입자 중 결국 누른 비율.
...
<section id="ctr-charts">                 ← MD §4-B-1
  ANDROID / IOS 두 막대 차트 side-by-side:
    - Y축: object_section_id (click_per_pv 낮은 순 정렬)
    - X축: click_per_pv (또는 토글로 CTR / IR / click_per_pv 선택)
    - 임계값 라인 (기본 1%) 미만은 빨간 막대
    - 호버 시 IR / CTR / click_per_pv / impression_uv / click_uv 툴팁
  토글 컨트롤: 보고싶은 지표 선택, 임계값 슬라이더
</section>

<section id="removal-candidates">         ← MD §5
  제거 후보 표:
    | element | object_type | ANDROID click/PV | IOS click/PV | 명세 여부 | 비고 |
    - 명세 only는 전부 포함 (사용도 0)
    - click_per_pv < 임계값 element 포함
    - 정렬: 가장 사용도 낮은 순
</section>
...
- 배경 `#141414`, 주색 `#2563EB` (정상) / `#EF4444` (경고/제거 후보) / `#10B981` (양호)
...
2. **Label Pool:**
   ...
   - chip 색상: click_per_pv 임계값 미만이면 빨간 테두리
...
## 1. 맥락
- 분석 목적: (예: "사용도 낮은 모듈 발굴 → 리뉴얼 시 제거 검토")
- 측정 기간:
...
## 5. 제거 후보
임계값: click_per_pv < 1% (사용자 조정 가능)
- old_promo_banner: 명세에는 있는데 측정 기간 동안 0회 클릭 → 즉시 제거 검토
- footer_link_legacy (ANDROID 0.3% / IOS 0.4%): 임계값 미만
- ...
