...
### user_id 찾는 법 (사용자가 모를 때 안내)

사용자가 자기 user_id를 모른다고 하면 아래 가이드를 한 줄로 알려준다:

> **오늘의집 웹사이트(https://ohou.se)**에서 로그인 → 우측 상단 프로필 → 마이페이지 진입 → 브라우저 주소창의 URL 맨 끝 숫자가 user_id야.
> 예: `https://ohou.se/users/4578208` → user_id = `4578208`

이 방법이 가장 빠르고 확실하다. 앱에서는 URL이 드러나지 않으므로 **반드시 웹에서 확인**하도록 안내.
