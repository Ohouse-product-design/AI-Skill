...
5. **ODS 근거 보강** — `ods-prototype` MCP가 있으면 조회해 용어·구조를 ODS 기준에 맞춘다. (아래 "ODS 참조" 참고)
6. 아래 포맷대로 description을 작성한다.
7. 추측한 부분은 `[확인 필요]`로 표시한다.
...
**출력 규칙 (중요):** ODS 조회 결과는 **용어·구조 파악을 위한 내부 참고**로만 쓴다. 명세(출력)에는 ODS 토큰명(예: `backgroundWeak`)·아이콘 에셋명(예: `[Asset] Sparkles Small`)·구체 스타일 값(hex, radius 등)을 **적지 않는다.** 오직 **컴포넌트가 실제로 특정 ODS 컴포넌트를 사용/구성한 경우에만** 그 ODS 컴포넌트명을 언급한다. (단순히 "비슷해 보인다"는 매핑 결과는 명세에 쓰지 않는다.)
